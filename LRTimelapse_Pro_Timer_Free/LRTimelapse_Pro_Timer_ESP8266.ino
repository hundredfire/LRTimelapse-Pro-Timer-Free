...
const String CAPTION = "Pro-Timer ESP8266";
const String CAPTION1 = "TLCS Edition ";

// ESP8266 (Wemos D1 Mini) Pin Definitions
// The LCD Keypad shield uses an analog pin to read the buttons.
// On the Wemos D1, this is the A0 pin.
...
const byte BACK_LIGHT = D1; // PWM-capable pin for dimming

// Camera 1 settings
// Connect to D2 and D0 on Wemos D1 Mini
#define Cam1_ext_HW
const byte Cam1_shoot = D2;
const byte Cam1_focus = D0;

// Camera 2 settings / Sensor
// Connect to TX and RX on Wemos D1 Mini
#ifdef sensor
#define Cam2_int_HW
#else
#define Cam2_int_HW
...
// EEPROM Settings
struct Settings {
  int BackgroundBrightnessLevel;
  float Interval;
  float MIN_DARK_TIME;
  float AUTO_FOCUS_TIME;
  byte CamWakeUptime;
  long displayRS;
};
...
void setup() {
    ...
    // check ranges
    settings.BackgroundBrightnessLevel = constrain(settings.BackgroundBrightnessLevel, cMinLevel, cMaxLevel);
    settings.Interval = constrain(settings.Interval, cMinInterval, cMaxInterval);
    settings.MIN_DARK_TIME = constrain(settings.MIN_DARK_TIME, min_MDT, max_MDT);
    settings.AUTO_FOCUS_TIME = constrain(settings.AUTO_FOCUS_TIME, min_AFT, max_AFT);
    settings.CamWakeUptime = constrain(settings.CamWakeUptime, 0, max_WAT);
    settings.displayRS = constrain(settings.displayRS, displayRSmin, displayRSoff);

    act_BackLightLevel = settings.BackgroundBrightnessLevel;
    interval = settings.Interval;
    MIN_DARK_TIME = settings.MIN_DARK_TIME;
    AUTO_FOCUS_TIME = settings.AUTO_FOCUS_TIME;
    CamWakeUptime = settings.CamWakeUptime;
    displayRS = settings.displayRS;
    ...
    // Init Timer for Interrupt Timing
...
*/
void DimLCD(int startval, int endval, byte stepdelay)
{
    if (endval < startval)
    {
    ...
    else
    {
        for (int bl = startval; bl <= endval; bl++)
        {
            analogWrite(BACK_LIGHT, bl); // dimming backlight on.
            delay(stepdelay);
            if (localKey != lastKeyPressed)
            {
                break;
...
*/
void changeBackLightBrightness(char AMode) { //U=up, D=down, R=rolling
    if ((AMode == 'U') || (AMode == 'D'))
    {
...
*/
int act_BackLightBrightness()
{
    // Scale brightness for ESP8266's 10-bit PWM range (0-1023)
    return constrain(act_BackLightLevel * (1023 / cMaxLevel), 48, 1023);
}
...
void save_Params()
{
    if (not isRunning)
    {
        ...
        if (act_BackLightLevel != settings.BackgroundBrightnessLevel) {
            settings.BackgroundBrightnessLevel = act_BackLightLevel;
            save = true;
        };
        ...
        if (MIN_DARK_TIME != settings.MIN_DARK_TIME) {
            settings.MIN_DARK_TIME = MIN_DARK_TIME;
            save = true;
        };
        if (AUTO_FOCUS_TIME != settings.AUTO_FOCUS_TIME) {
            settings.AUTO_FOCUS_TIME = AUTO_FOCUS_TIME;
            save = true;
        };

        if (CamWakeUptime != settings.CamWakeUptime) {
            settings.CamWakeUptime = CamWakeUptime;
            save = true;
        };

        if (displayRS != settings.displayRS) {
            settings.displayRS = displayRS;
            save = true;
        };
